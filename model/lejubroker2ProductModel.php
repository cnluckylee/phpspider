...
class lejubroker2ProductModel extends productXModel {
    ...
    public function getCommon()
    {
        ...
        foreach ($nodes as $k=>$node) {
            ...
            {
                $this->_common[$key] = str_replace(array("服务区域：","所在门店：","服务区域："),"",$this->_getRegexpInfo($filter, $this->getContent()));
            }
        }
    ...
    {
        $nodes = parent::getCategoryItemDprice();
        $filter2 = './/dd/p[3]/a/text()||2';
    ...
    public function getCategoryItemDistrict()
    {
        $filter = $this->_config[\elements::CATEGORY_ITEM_DPRICE];
        $nodes = $this->_xpath->query($filter);
        $filter2 = $this->_config[\elements::CATEGORY_ITEM_DISTRICT];
        $this->_category_item_district = array();
        foreach ($nodes as $k=>$node) {
            foreach ($this->_xpath->query($filter2, $node) as $child) {
                $str = $child->nodeValue;
                $this->_category_item_district[$k][] = $str;
            }

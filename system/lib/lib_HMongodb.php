...
class HMongodb {
	 ...
	 * 失败：false
	 */
	function init($mongo_server = null, $connect = true, $auto_balance = true) {
		...
		try {
			$this->mongo = new MongoClient ( $mongo_server, array (
					'connect' => $connect 
			) );
	...
	function insert($table_name, $record) {
		$dbname = $this->curr_db_name;
		try {
			$this->mongo->$dbname->$table_name->insert ( $record, array (
					'w' => true ,
					'w'=>1
			) );
			return true;
...
//            print_r($record);
	 ...
	 * 失败：false
	 */
	function batchinsert($table_name, $record) {
		$dbname = $this->curr_db_name;
		try {
			$string = $record[0]['title'];
			if(mb_detect_encoding($string, 'UTF-8') !== 'UTF-8')
			exit("charset error!,no insert");
			$this->mongo->$dbname->$table_name->batchInsert ( $record, array (
					'w' => true ,
					'safe'=>true,
